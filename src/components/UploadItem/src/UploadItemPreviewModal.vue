...
<script lang="ts">
  ...
  import FileList from './FileItemList.vue';
  import { BasicModal, useModalInner } from '/@/components/Modal';
  import { previewProps, UploadFileItem } from './props';
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      // 删除
      function handleRemove(record: UploadFileItem) {
        ...
        if (index !== -1) {
          ...
          emit(
            'list-change',
            fileListRef.value.map((item) => item)
          );
        }
      }

    // 预览
    function handlePreview(record: UploadFileItem) {
    ...
    function openVideoPreview(url: string, title: string) {
      ...
      // 下载
      function handleDownload(record: UploadFileItem) {
        let { url = '' } = record;
      ...
      return {
        ...
        closeModal,
        fileListRef,
        columns: createPreviewColumns({handlePreview}) as any[],
