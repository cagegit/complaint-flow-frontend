...
// 文件预览列表
export function createPreviewColumns({handlePreview}: {handlePreview: Fn}): BasicColumn[] {
  return [
    {
      dataIndex: 'url',
      title: t('component.upload.legend'),
    ...
    },
    {
      ...
      customRender: ({ text, record }) => {
        if(handlePreview) {
          return (
            <a href="javascript:void(0);" class="cursor-pointer" onClick={() => handlePreview(record as PreviewFileItem)}>
              {text}
            </a>
          );
        } else{
        return <span class="truncate" title={text}>{text}</span>;
        }
      }
    },
  ];
