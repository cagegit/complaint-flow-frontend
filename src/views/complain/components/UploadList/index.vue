  ...
  <div class="upload-list-container">
      ...
      >
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'fileTagType'">
            <a-select v-model:value="record.fileTagType" style="width: 100%" placeholder="请选择市级附件类型" :options="cityFileTypeOptions">
            </a-select>
          </template>
    ...
    >
      <div class="upload-modal-content">
        <a-table :dataSource="uploadedFiles" :columns="uploadColumns" :pagination="false" :rowKey="(record) => record.uid || record.id">
          <!-- 文件名列 -->
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'fileName'">
            ...
            <!-- 市级附件类型列 -->
            <template v-if="column.key === 'fileTagType'">
              <a-select v-model:value="record.fileTagType" style="width: 100%" placeholder="请选择" :options="cityFileTypeOptions" />
            </template>

            <!-- 区级附件类型列 -->
            <template v-if="column.key === 'districtFileTagType'">
              <a-select v-model:value="record.districtFileTagType" style="width: 100%" placeholder="请选择" :options="districtFileTypeOptions" />
            </template>
    ...
    <!-- 文件预览弹窗 -->
    <a-modal v-model:visible="previewModalVisible" :title="previewFile?.fileName || '文件预览'" :width="800" :footer="null" :destroyOnClose="true">
      <div class="preview-container flex justify-center" style="max-height: 750px; overflow: auto">
        <!-- 图片预览 -->
        <img v-if="isImageFile(previewFile)" :src="previewUrl" class="max-w-full max-h-full mx-auto" alt="预览图片" />
        <!-- PDF预览 -->
        <iframe v-else-if="isPdfFile(previewFile)" :src="previewUrl" class="w-full h-full" frameborder="0"></iframe>
        <!-- 音频预览 -->
        <audio v-else-if="isAudioFile(previewFile)" controls="true" autoplay="true" class="w-full">
          <source :src="previewUrl" />
          您的浏览器不支持音频播放
        </audio>
        <!-- 视频预览 -->
        <video v-else-if="isVideoFile(previewFile)" controls="true" class="w-full h-full">
          <source :src="previewUrl" />
          您的浏览器不支持视频播放
        </video>
        <!-- 其他文件 -->
    ...
    <!-- 从回复记录的附件里选择文件 -->
    <a-modal v-model:visible="replyFileListModalVisible" :title="'回复记录附件选择'" :width="800" :footer="null">
...
<script setup lang="ts" name="UploadList">
  ...
  //@ts-ignore
  import { PlusOutlined, DownOutlined, UpOutlined, InboxOutlined, PaperClipOutlined, FileOutlined } from '@ant-design/icons-vue';
  ...
  const props = defineProps({
    ...
    accept: {
      type: String,
      default:
        '.pdf,.doc,.docx,.xls,.xlsx,.txt,.png,.jpg,.jpeg,.aac,.wma,.cda,.flac,.mid,.mka,.mp2,.mpa,.mpc,.ape,.ofr,.ogg,.ra,.wv,.tta,.ac3,.dts,.mp3,.mp4,.m4a,.wav',
    },
    // 回复附件列表
    replyFileList: {
      type: Array as PropType<FileItem[]>,
  ...
  function handleSelectReplyFiles() {
    ...
    // 将选中的回复记录附件添加到文件列表
    const selectedFiles = props.replyFileList.filter((file) => selectedReplyRowKeys.value.includes(file.fileKey));
  ...
  const beforeUpload = (file: File) => {
    ...
    const acceptTypes = props.accept.split(',').map((type) => type.trim().toLowerCase().replace('.', ''));
  ...
  const handleUploadOk = () => {
    ...
    // 检查是否都选择了类型
    const hasEmptyType = uploadedFiles.value.some((file) => file.fileTagType === null || file.districtFileTagType === null);
  ...
  const handleRemoveUpload = (file) => {
    uploadedFiles.value = uploadedFiles.value.filter((item) => item.uid !== file.uid && item.id !== file.id);
  };
  ...
  const handleDelete = (file) => {
    ...
    fileList.value = fileList.value.filter((item) => (item.id && item.id !== file.id) || (item.uid && item.uid !== file.uid));
